...
2. **Избыточные подключения к БД в локальной сети**
   - **Проблема**: В исходной конфигурации файлы `pg_hba.conf` и `postgresql.conf` не ограничивали подключения, что позволяло подключаться к БД из локальной сети. Порт открыт наружу через Docker Compose.
   - **Решение**: Изменены конфигурационные файлы `pg_hba.conf` и `postgresql.conf`, которые монтируются в Docker-контейнер и заменяют исходные. Ограничены подключения только из доверенных источников.
